...
// ---- Panel-level prompt (unanswered state) ----
  ...
  <div className="pm-section pm-section--prompt">
    ...
    <div className="pm-prompt__actions">
      <button className="pm-prompt__btn pm-prompt__btn--yes" type="button"
        onClick={() => onAnswer("yes")}>Yes</button>
      <button className="pm-prompt__btn pm-prompt__btn--no" type="button"
        onClick={() => onAnswer("no")}>No</button>
    </div>
  </div>
);

// ---- Panel-level no-pipe card (no state) ----

const NoPipeCard: React.FC<{
  onSwitchToYes: () => void;
}> = ({ onSwitchToYes }) => (
  <div className="pm-section pm-section--no-pipe">
    <p className="pm-no-pipe__message">パイプはありません</p>
    <button className="pm-no-pipe__switch-btn" type="button" onClick={onSwitchToYes}>
      変更する
    </button>
  </div>
);
...
// ---- Collapsed section (yes + !isActive) ----
...
const PipeMeasurement: React.FC<PipeMeasurementProps> = ({
  sections, pipeAnswer, focusedField,
  onUpdate, onDelete, onAdd, onAnswer,
  onFieldFocus, onFieldBlur, onFieldHover, onFieldHoverEnd,
}) => {
  const activePipes = sections.filter((s) => s.isActive).length;
  ...
  return (
    <div className="pm-panel">
      ...
      {pipeAnswer === "unanswered" && (
        <PromptCard onAnswer={onAnswer} />
      )}

      {pipeAnswer === "no" && (
        <NoPipeCard onSwitchToYes={() => onAnswer("yes")} />
      )}

      {pipeAnswer === "yes" && sections.map((s) =>
        s.isActive ? (
          <SectionCard
